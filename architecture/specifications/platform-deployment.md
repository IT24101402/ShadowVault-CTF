...
- `docker_shadowvault_db_data`
- `docker_shadowvault_redis_data`
- `docker_shadowvault_ctfd_uploads`
- `docker_shadowvault_ctfd_logs`

The `docker_` prefix is generated by the current Docker Compose project naming configuration.

Verified mount locations:

- `docker_shadowvault_db_data` → `/var/lib/mysql`
- `docker_shadowvault_redis_data` → `/data`
- `docker_shadowvault_ctfd_uploads` → `/var/uploads`
- `docker_shadowvault_ctfd_logs` → `/var/log/CTFd`
...
The following resources are estimated for the development and testing deployment of Operation ShadowVault.

### CPU

- Minimum: 2 virtual CPU cores
- Recommended: 4 virtual CPU cores
- CPU usage will increase when multiple interactive challenge containers are running simultaneously.

### Memory

- Minimum: 4 GB RAM
- Recommended: 6 GB RAM or more
- CTFd, MariaDB, and Redis have relatively modest memory requirements.
- Additional memory is required when Stages 03, 05, and 06 are running concurrently.

### Storage

- Minimum available storage: approximately 15 GB
- Recommended available storage: 20 GB or more
- Storage is required for Docker images, persistent platform volumes, challenge artifacts, logs, and container layers.

### Network

- Local TCP connectivity between the participant machine and the Kali VM is required.
- TCP port 8000 is currently published for CTFd.
- Interactive challenge ports will be published only when required by the final challenge implementations.
- MariaDB port 3306 and Redis port 6379 remain internal and are not published to participants.
- External Internet access is not required for solving the intended CTF challenges after required images and dependencies are available locally.
...
Participant-side analysis may use security tools appropriate to each challenge, including tools available within Kali Linux.

### Architecture Compatibility

The primary development host currently uses ARM64 (aarch64).

Challenge containers should use multi-architecture base images where practical to improve compatibility across team development systems.

Architecture-specific binaries must be documented and tested against their intended execution architecture before final integration.
